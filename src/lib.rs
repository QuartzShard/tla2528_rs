...
impl<I2C: i2c::I2c> Tla2528<I2C> {
    ...
    pub fn new(addr: u8, i2c: I2C, config: Config) -> Result<Self, TlaError<I2C>> {
        ...
        let status = SystemStatus::from(adc.read_reg(SYSTEM_STATUS)?);
    ...
    pub fn analog_read_manual(&mut self, channel: Channel) -> Result<u16, TlaError<I2C>> {
        ...
        if let SeqMode::Automatic = self.config.sequence.seq_mode() { return Err(TlaError::WrongMode) };
        if self.config.pin.val_0_at(channel as usize) { return Err(TlaError::AnalogFromDigital)}
    ...
    pub fn analog_read_auto<'buf>(&mut self, reads: &'buf mut [(u16, Option<Channel>)]) -> Result<&'buf [(u16, Option<Channel>)], TlaError<I2C>> {
        if let SeqMode::Manual =  self.config.sequence.seq_mode() { return Err(TlaError::WrongMode) };
    ...
    fn analog_read(&mut self) -> Result<(u16, Option<Channel>), TlaError<I2C>> {
        ...
        let averaging = if let OversamplingRatio::_0 = self.config.osr.osr() { false } else { true };
        ...
        self.i2c.read(self.addr, buf).map_err(|e| TlaError::I2c(e))?;
    ...
    pub fn digital_in(&mut self, channel: Channel) -> Result<bool, TlaError<I2C>> {
        let channel = channel as u8;
        if !self.config.pin.val_0_at(channel as usize) { return Err(TlaError::DigitalFromAnalog)};
        if self.config.gpio.val_0_at(channel as usize) { return Err(TlaError::WrongGPIODirection)};
    ...
    fn write_reg(&mut self, addr: u8, val: u8) -> Result<(), TlaError<I2C>> {
        self.i2c.write(self.addr, &[Opcode::SingleWrite as u8, addr, val]).map_err(|e| TlaError::I2c(e))?;
        Ok(())
    }
    ...
    fn set_bit(&mut self, addr: u8, bitmask: u8) -> Result<(), TlaError<I2C>> {
        self.i2c.write(self.addr, &[Opcode::SetBit as u8, addr, bitmask]).map_err(|e| TlaError::I2c(e))?;
        Ok(())
    }
    ...
    fn clear_bit(&mut self, addr: u8, bitmask: u8) -> Result<(), TlaError<I2C>> {
        self.i2c.write(self.addr, &[Opcode::ClearBit as u8, addr, bitmask]).map_err(|e| TlaError::I2c(e))?;
        Ok(())
    }
